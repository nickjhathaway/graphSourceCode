...
if [ $(uname) == "Linux" ]; then
  for x in $(find $1 -iregex '.*\(\.c\|\.cpp\|\.h\|\.hpp\)'); do
    if [[ $(basename $x) == *.h*  ]]
    then
      echo $(echo $(basename $x) | sed 's/\./_/g') [shape=circle,style=filled,fixesize =true, color = \"#000000\", fillcolor =\"#0571b0\", width = 1]>>$outFile ;
    else
      echo $(echo $(basename $x) | sed 's/\./_/g') [shape=circle,style=filled,fixesize =true, color = \"#000000\", fillcolor =\"#ca0020\", width = 1]>>$outFile ;
    fi
    done
    for x in $(find $1 -iregex '.*\(\.c\|\.cpp\|\.h\|\.hpp\)'); do for y in $(echo $(egrep "#include" $x | grep -oE '["].*\.h.*["]') | sed '/^$/d'| sed -e 's/\b"//g'  -e 's/"\b//g'); do
    if [[ $(basename $x) == *.h*  ]]
    then
      echo $(echo $(basename $y) \-\> $(basename $x) | sed 's/\./_/g') [penwidth=5, color=\"#92c5de\"] >>$outFile;
    else
      echo $(echo $(basename $y) \-\> $(basename $x) | sed 's/\./_/g') [penwidth=5, color=\"#f4a582\"] >>$outFile;
    fi
    done ;
  done
else
